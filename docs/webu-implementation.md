...
- **抓取中的 spinner**（2026-09-22）：`theme.spinnerFrames`（braille，`spinnerFrame()` 由時鐘算 frame、不累加計數，所以重複的 tick 只多一次重畫、不會跳格）取代 URL 列的 `glyphWeb`；glyph 與 URL 同用 `urlColor`；`spinTickMsg` / `spinCmd()`（`spinStep` 90ms）在 `Update` 的 `tea.KeyMsg`（使用者發動的導覽）與 `pageEventMsg`（頁面自己發動的）兩處武裝，之後靠 `m.fetching()`（任何 tab `loading`）自我續命
