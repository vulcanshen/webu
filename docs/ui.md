...
- **navigation / breadcrumb 是一行入口**（2026-09-21，取代原本的「導覽清單一行流式」）：`▎ 󰍜 目前所在 +N`（navigation 寫目前的 tab；breadcrumb 用 󰴠、寫最後一節；不寫 landmark 名字），左邊直槓（Blue）、文字帶底色（同 code block 的 surface0）；連結不畫，Enter 開 item operation 選單列出來
