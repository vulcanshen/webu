...
- **tray（膠囊列）**（2026-09-22，入口行搬離頁面）：`layout.tray []capsule`（`kind` + `nodes` + label）：`renderer.chrome` 由 `block` 的 Landmark 分支與 `inline` 的 skip link 分支收集、`renderer.other` 由 `page()` 收有 main 時 main 之外不在 landmark 裡的節點（`holdsLandmark` 的 wrapper 走進去），`buildTray` 依 `trayKindOf` 分桶成固定順序 skip / header / nav / search / sidebar / footer / dialog×N / other；label = `capsuleText`：`glyphMenu` + `capsule.title()`（`trayKind.word()`，dialog 用 `entryLabel` 的名字）+ `+len(capsuleTargets)`；hint = `capsuleHint`（role 名字 / N navigations · at 目前所在（`entryCurrent`）· N inside）；選單 = `capsuleMenuItems`（多個 landmark 時每個一個 header 列、`targetRow` 的 key 用跨 landmark 的流水號、aria-current 的 hint 標 here）；`enterCapsule`：沒目標 → `showCapsule` 全文、skip 一個目標 → `actOn`（skip link 跳不到就 `skipToContent`）、search 一個框 → `editFieldAs`、其餘 `openItemMenu`；`dispatch` 的 `entry:i` 走 `tab.currentTargets()`；main 不畫 rule 也不是 item（`firstItem` 改用 `marks[main]`）、`sectionheader` / `sectionfooter` → Group、`isSkipLink` 加 jump to；`Esc`（沒 float / mode、focus 在頁面）→ `enterTray` / `leaveTray`；不再 emit 列、不佔 item、不進 `marks`；`layout.fit = fitTray(tray, width)`（全部塞得下就全顯示，否則留位給 `+N`）；`tab.tray` 編碼 0 = 在頁面、i+1 = 第 i 顆、`trayMore` = `+N`（`onTray` / `onMore` / `trayIndex` / `traySlot` / `traySlots` / `focusTray` / `focusMore` / `leaveTray` / `enterTray` / `stepTray` / `capsuleOf`）；`tab.current()` 先看膠囊、`+N` 上是 nil；`curFolded` 取代直接讀 `items[cursor].folded`；`moveItem`：頁面最上列再 `k` → `enterTray`、`h`/`l` → `stepTray`（繞）、`j` → `leaveTray`、其他鍵先離開再照舊；`pagepanel.trayRow` 畫在 URL 下那列（`capsuleChip`：`capLeft` + label + `capRight`，surface0 / handColor / borderDim、`+N` 字 dim），hand 在膠囊列時頁面游標降成 curOff；`+N` Enter → `openTrayMore`（`optTrayMore`、key `tray:i`）→ `focusTray(i)`（該顆暫佔最後一格，`traySlots`）+ `openItemMenuAt`（沿用同一層）；`apply` 重抓後依 backendDOMNodeId 把 hand 放回同一顆、`relayout` 後越界就 `leaveTray`；Outline 選到入口 landmark → `focusTray`；`jumpToAnchor` / `jumpTo` / select mode 進出 / `cursorOn` 都 `leaveTray`；`isSkipItem` / `entryRow` / `entryLine` / `skipRow` / `segNavBar` / `segNav` 拿掉，`firstItem` 不再需要跳過 skip
