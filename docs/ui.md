...
- **兩套配色，不是一套**（2026-09-22，user 定案）：**app 配色**是 webu 自己的外殼（面板、邊框、header 鏈、pagetab、游標、選單、popup、footer），完整吃 VTP——少數錨點、明度當 z-axis、一個語意一條色帶、override 色不參與層級；**page 配色**是文件自己的結構（heading、連結、code、表格、引言、欄位），是另一套封閉集合，只出現在 `[2]` 的內容區、不參與 webu 的 z-axis（頁面永遠在同一層：canvas）、色帶只在自己這套裡互相專職。理由：網頁本身就是結構化格式、有自己的視覺語言，webu 不吃 CSS 但仍要畫那個結構；把它塞進 app 的色帶裡，就會一天到晚要在「表頭要 mauve」與「code key 也要 mauve」、「highlight 要 lavender」與「selection 也要 lavender」之間二選一。兩套今天撞到同一個 hex，是兩個獨立決定剛好落在同一色，不是一條色帶用兩次。程式碼裡 app 的叫 `focusColor` / `handColor` / `headerColor` …，page 的一律 `page` 開頭（`pageText` / `pageLink` / `pageCode` / `pageTableBg` / `pageHeadTop` …）
