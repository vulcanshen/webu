...
- 入口型 landmark（banner / navigation / breadcrumb / search / complementary / contentinfo）：一行入口 `▎ icon 字 +N`（icon 分種類；字是「你在裡面的哪一格」或它是什麼，不寫 role），內容不進畫面；Enter = 它的 item operation 選單（裡面每個 link / button / 輸入框 / 勾選 / select 一列、巢狀清單縮排、Enter 就做該物件的事）——「Enter = 該物件的期望操作」在入口型物件上的樣子；search 只有一個框時 Enter 直接開框；Space 照舊 item operation + panel operation（2026-09-21）
...
| 入口型 landmark：banner / navigation / breadcrumb / search / complementary / contentinfo | **一行入口** `▎ icon 字 +N`（2026-09-21，user 定案：這一套 style 套到所有 page chrome，只用 icon 分種類；main / article / region / form 是內容，維持分隔線 + 可收合）。icon：banner page_layout_header、navigation menu、breadcrumb map_marker_path、search search、complementary table_of_contents（側欄多半是 TOC）、contentinfo page_layout_footer；dialog / alertdialog（cookie 同意、登入 modal，非 landmark 的雜訊也走這條——user 2026-09-21：用 popup menu 呈現）dock_window，字是 aria-label → 第一個 heading → 第一段文字，Enter 列出它的按鈕；**skip link**（「Skip to main content」那種：同頁 `#` 錨點且文字以 skip 開頭、或 DOM class 含 skip）也是一行入口，icon skip_next、字就是它的文字；**skip 區塊**（Atlassian 的「Skip to: Top Bar / Sidebar / Main Content」：class / id / aria-label 含 skip 的容器、裡面全是同頁錨點）整個包成一個入口 `▎ 󰒭 Skip to: +3`，Enter 列出目標（user 2026-09-21：跟收納 style 一樣、整個 skip to 是一個項目）。Enter / 選了目標 = 它的意思：**跳到錨點**——`jumpToAnchor` 用 DOM snapshot 的 id 與 parent 表，游標停到錨點元素（它本身是 item）或它裡面第一個 item；錨點找不到東西可停時，單一 skip link 退回「內文第一個 item」、其他就交給頁面 click。**同頁錨點連結一律這樣跳**（TOC 也是），不 confirm、不換頁。新頁面的起點永遠跳過 skip link 與 skip 區塊；banner 等入口的選單裡不列單一 skip link。字——**不寫 landmark 的 role / 名字，寫「你在裡面的哪一格」或它是什麼**：navigation 是目前的 tab（`aria-current` → URL 比對：連結 URL 等於頁面 URL、或最長路徑前綴 → 才退回 landmark 名字）、breadcrumb 是最後一節、banner 是站名（第一個不是 `#` skip link 的連結文字，logo）、search 是框的名字、complementary / contentinfo 是 landmark 名字 → 第一個 heading → 沒有就只剩 icon。`+N` = 裡面的 link / button / 輸入框 / 勾選 / select 數。內容不進畫面、不佔橫向空間（橫向不定長的東西會誘導 `h/l`，terminal 最忌諱；`j/k` 一步跳過）。`aria-current` 與 breadcrumb 標記 AX tree 沒有，從 DOM snapshot 讀；沒包在 nav 裡的 `ol.breadcrumb` 在 ir 包成一個 navigation landmark。Enter = item operation 選單：裡面每個目標一列（巢狀清單縮排；link 標 URL、button / 輸入框 / 勾選 / select 各標種類）、Enter 就做該物件的事（link / button / 勾選 = click、不再 confirm，輸入框 = 開框，select = 開清單）；**search 只有一個框時 Enter 直接開框**；Space = 同一份 item operation + panel operation；Yank text、Inspect 照舊；不收合（沒東西可收）。取代原本的「導覽清單一行流式」與這些 landmark 的分隔線 |
