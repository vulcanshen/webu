...
- 入口型 landmark（banner / navigation / breadcrumb / search / complementary / contentinfo）：URL 底下分隔線上的膠囊 `icon 字 +N`（2026-09-22 從頁面裡的一行搬上去；icon 分種類；字是「你在裡面的哪一格」或它是什麼，不寫 role），內容不進畫面、頁面本體從內容開始；塞不下的收成最後一顆 `+N`、Enter 列出；頁面最上列的 item 按 `k` 上去、`h`/`l` 走（會繞）、`j` 回來；Enter = 它的 item operation 選單（裡面每個 link / button / 輸入框 / 勾選 / select 一列、巢狀清單縮排、Enter 就做該物件的事）——「Enter = 該物件的期望操作」在入口型物件上的樣子；search 只有一個框時 Enter 直接開框；Space 照舊 item operation + panel operation（2026-09-21）
...
| 鍵 | 動作 |
|---|---|
| `j` / `k` | 上下一列，**清單會繞**（`[1]` 與所有 menu popup）；`[2]` item 游標**不繞**（頁面有頭尾），且是**換列**：跳到下一列有 item 的列、落在欄位最接近的 item（修訂 2026-09-20：一列多個 item 之後，一列一列走才走得動）；頁面最上列再按 `k` 上到 URL 底下分隔線上的膠囊列、`j` 回到剛才的 item（2026-09-22） |
| `u` / `d` | 半頁，不繞 |
| `gg` / `G` | 頭 / 尾 |
| `h` / `l` | `[2]` 同一列的 item 之間左右移動（表格 row、導覽列、一段文字裡的多個連結都一樣；列尾不繞）；膠囊列上走膠囊、**會繞**（它是選單，2026-09-22）；DevTools popup 切分頁；其他地方無 |
| 方向鍵 | 同義 |
| `1`–`2` | 直達面板 |
