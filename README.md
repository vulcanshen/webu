...
**`[W]eb`** — `[1] Tabs` beside `[2] Page`. The tabs list is one row per Chromium target; the cursor says where you are, green says which one the page panel is showing, and two tabs on the same URL are numbered in the order they were opened. The page panel is always the page: its first row is the URL, then the page as **items** — links, buttons, text boxes, checks, selects, media, headings, landmarks — with paragraphs flowing between them at a measured width. `j`/`k` step by row and `h`/`l` along one, so a row of links is walked sideways rather than skipped. Landmarks open as a named rule (`▾ banner ────`) and collapse on `Enter`; so do headings, down to the next heading of their level. The page's chrome — header, navigation, breadcrumb, search, sidebar, footer — is off the page, on the rule under the URL, one capsule each with a glyph for its kind and a word for where you are in it: ` 󰍜 Docs +12 ` is the tab you are on and how many links are behind it, and the ones the width leaves out sit behind a `+N`. `k` from the top of the page goes up to them, `h`/`l` walk them, `j` comes back; `Enter` lists what one holds, `Enter` again opens it; a search box opens to type, and what you type is offered to the page's Enter. A dialog — a cookie banner — is a capsule of the same kind, and so is a skip link, which does what it says; any link into the page lands the cursor on what it names. A JSON, YAML, TOML, Markdown or plain-text response is drawn as one code block with syntax colour instead of Chrome's own viewer.
