...
- **膠囊列**（2026-09-22，入口行搬離頁面）：`layout.bar []capsule`（node + label = `capsuleText` / `skipText`）由 `renderer.block` 的 Landmark 分支與 `inline` 的 skip link 分支收集，不再 emit 列、不佔 item、不進 `marks`；`layout.fit = fitBar(bar, width)`（全部塞得下就全顯示，否則留位給 `+N`）；`tab.bar` 編碼 0 = 在頁面、i+1 = 第 i 顆、`barMore` = `+N`（`onBar` / `onMore` / `barIndex` / `barSlot` / `barSlots` / `focusBar` / `focusMore` / `leaveBar` / `enterBar` / `stepBar` / `capsuleOf`）；`tab.current()` 先看膠囊、`+N` 上是 nil；`curFolded` 取代直接讀 `items[cursor].folded`；`moveItem`：頁面最上列再 `k` → `enterBar`、`h`/`l` → `stepBar`（繞）、`j` → `leaveBar`、其他鍵先離開再照舊；`pagepanel.barRow` 畫在 URL 下那列（`capsuleChip`：`capLeft` + label + `capRight`，surface0 / handColor / borderDim、`+N` 字 dim），hand 在膠囊列時頁面游標降成 curOff；`+N` Enter → `openBarMore`（`optBarMore`、key `bar:i`）→ `focusBar(i)`（該顆暫佔最後一格，`barSlots`）+ `openItemMenuAt`（沿用同一層）；`apply` 重抓後依 backendDOMNodeId 把 hand 放回同一顆、`relayout` 後越界就 `leaveBar`；Outline 選到入口 landmark → `focusBar`；`jumpToAnchor` / `jumpTo` / select mode 進出 / `cursorOn` 都 `leaveBar`；`isSkipItem` / `entryRow` / `entryLine` / `skipRow` / `segNavBar` / `segNav` 拿掉，`firstItem` 不再需要跳過 skip
