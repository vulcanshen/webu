...
- 入口型 landmark（banner / navigation / breadcrumb / search / complementary / contentinfo）：URL 底下那條 **pagetab**（user 命名，2026-09-22）上的一節 `字 +N`（一種一節、順序固定 skip / header / nav / search / sidebar / footer / dialog / other，字是種類的固定字彙、dialog 用它的名字、**只有整條鏈的開頭掛一個漢堡 icon**、每一節自己沒有——全部併成一條 powerline 鏈，就是 header `[W]eb` 那條的畫法；「你在裡面的哪一格」放面板下框的 hint），內容不進畫面、頁面本體從內容開始；塞不下的收成最後一顆 `+N`、Enter 列出；`Esc`（沒 popup / mode 時）跳上 pagetab、再 `Esc` 回頁面（Space menu 的 panel operation 有這一列：往上是 `[Esc] Page chrome`、在上面時是 `[Esc] Back to the page`——**core key（Enter / Esc / `/`）一律把鍵寫進 label 自己**（2026-09-22）：`bracketHotkey` 只能就地括住 label 裡的一個字母，core key 沒有字母可括，而一個面板操作沒被揭露就等於不存在（§A.1）。同規則的還有 `[Enter] Switch to`（`[1]` 分頁）、`[Enter] Expand` / `[Enter] Collapse` / `[Enter] Open in new tab`（list screen 各列）、`[/] Search`（page）、`[/] Filter`（list screen） —— 面板操作沒進 menu 就等於不存在，§A.1）；頁面最上列的 item 按 `k` 也上得去、`h`/`l` 走（會繞）、`j` 回來；Enter = 它的 item operation 選單（裡面每個 link / button / 輸入框 / 勾選 / select 一列、巢狀清單縮排、Enter 就做該物件的事）——「Enter = 該物件的期望操作」在入口型物件上的樣子；search 只有一個框時 Enter 直接開框；Space 照舊 item operation + panel operation（2026-09-21）
...
| 入口型 landmark：banner / navigation / breadcrumb / search / complementary / contentinfo | **URL 底下 pagetab 上的一節** `字 +N`（2026-09-22：從頁面裡的一行搬到分隔線上、頁面本體從內容開始；**一種一節**、順序固定 skip / header / nav / search / sidebar / footer / dialog（每個 dialog 一顆、用名字）/ other（有 main 時、main 之外又不在任何 landmark 裡的東西：促銷條、純 div 的 cookie 橫幅）；字是種類的固定字彙——真站上「你在哪一格」一半是 Homepage、(Top)、PRODUCTS 這種垃圾、站名又跟 URL 重複，所以「你在哪一格」改放面板下框的 hint（`navigation Repository · at Code · 7 inside`、`3 navigations · 21 inside`）；**全部併成一條鏈、漢堡 icon 只掛在鏈的開頭一次**（每一節都是清單，各掛一個說不出差別又吃三格；整條是一個選單，所以說一次就好——user 2026-09-22；鏈的畫法與 header 的 `[W]eb` 那條同一套：圓帽起訖、相鄰用斜接縫、hand 停的那節點亮）；`+N` = 裡面的目標數；Enter 清單用 header 列分段（每個 nav 一段）、aria-current 的列 hint 標 here；一個目標都沒有的那一節 Enter 看全文；`Esc` 上 pagetab / 回頁面；塞不下的收成最後一顆 `+N`；2026-09-21 user 定案：這一套 style 套到所有 page chrome，只用 icon 分種類；main / article / region / form 是內容，維持分隔線 + 可收合）。icon（2026-09-22 起先全部改成漢堡選單 menu、同日再整個拿掉，種類靠字與位置；原本：banner page_layout_header、navigation menu、breadcrumb map_marker_path、search search、complementary table_of_contents、contentinfo page_layout_footer）；dialog / alertdialog（cookie 同意、登入 modal，非 landmark 的雜訊也走這條——user 2026-09-21：用 popup menu 呈現）dock_window，字是 aria-label → 第一個 heading → 第一段文字，Enter 列出它的按鈕；**skip link**（「Skip to main content」那種：同頁 `#` 錨點且文字以 skip 開頭、或 DOM class 含 skip）也是一節，icon skip_next、字就是它的文字；**skip 區塊**（Atlassian 的「Skip to: Top Bar / Sidebar / Main Content」：class / id / aria-label 含 skip 的容器、裡面全是同頁錨點）整個包成一節 `󰒭 Skip to: +3`，Enter 列出目標（user 2026-09-21：跟收納 style 一樣、整個 skip to 是一個項目）。Enter / 選了目標 = 它的意思：**跳到錨點**——`jumpToAnchor` 用 DOM snapshot 的 id 與 parent 表，游標停到錨點元素（它本身是 item）或它裡面第一個 item，**並把它捲成畫面第一列**（2026-09-22；hand 在 pagetab 上時一併回到頁面）；錨點找不到東西可停時，單一 skip link 退回「內文第一個 item」、其他就交給頁面 click。**同頁錨點連結一律這樣跳**（TOC 也是），不 confirm、不換頁。新頁面從內容開始（chrome 都不在頁面裡）；banner 等入口的選單裡不列單一 skip link。hint 的字（2026-09-22 前是那一節的字；節上的字改成種類固定字彙後這套規則搬到面板下框的 hint）——**不寫 landmark 的名字，寫「你在裡面的哪一格」或它是什麼**：navigation 是目前的 tab（`aria-current` → URL 比對：連結 URL 等於頁面 URL、或最長路徑前綴 → 才退回 landmark 名字）、breadcrumb 是最後一節、banner 是站名（第一個不是 `#` skip link 的連結文字，logo）、search 是框的名字、complementary / contentinfo 是 landmark 名字 → 第一個 heading → 沒有就只剩 icon。`+N` = 裡面的 link / button / 輸入框 / 勾選 / select 數。內容不進畫面、不佔頁面；pagetab 本身是選單、不是一行字：固定順序、塞不下就 `+N`（Enter 列出，選一節就把 hand 放上去、它暫佔最後一格、並開它的清單），絕不橫向捲動（橫向不定長的東西會誘導 `h/l`，terminal 最忌諱）；`k` 從頁面最上列上去、`h`/`l` 走（會繞）、`j` 回剛才的 item、其他移動鍵先回頁面。`aria-current` 與 breadcrumb 標記 AX tree 沒有，從 DOM snapshot 讀；沒包在 nav 裡的 `ol.breadcrumb` 在 ir 包成一個 navigation landmark。Enter = item operation 選單：裡面每個目標一列（巢狀清單縮排；link 標 URL、button / 輸入框 / 勾選 / select 各標種類）、Enter 就做該物件的事（link / button / 勾選 = click、不再 confirm，輸入框 = 開框，select = 開清單）；**search 只有一個框時 Enter 直接開框**；Space = 同一份 item operation + panel operation；Yank text、Inspect 照舊；不收合（沒東西可收）。取代原本的「導覽清單一行流式」與這些 landmark 的分隔線；2026-09-22 再從頁面裡的一行搬到 URL 底下的分隔線上 |
...
| heading | `#`×層級 + 標題文字，**整列鋪一層底色**（2026-09-22）：h1 最明顯、h6 最淡，六級由 surface2 lerp 到 crust（VTP §2.5 的插值機制，同 popup layer 邊框的作法）。收合的畫成 `▸ # 標題 · N items` |
...
| 鍵 | 動作 |
|---|---|
| `j` / `k` | 上下一列，**清單會繞**（`[1]` 與所有 menu popup）；`[2]` item 游標**不繞**（頁面有頭尾），且是**換列**：跳到下一列有 item 的列、落在欄位最接近的 item（修訂 2026-09-20：一列多個 item 之後，一列一列走才走得動）；頁面最上列再按 `k` 上到 URL 底下的 pagetab、`j` 回到剛才的 item；`Esc` 也上得去、再 `Esc` 回來（2026-09-22） |
| `u` / `d` | 半頁，不繞 |
| `gg` / `G` | 頭 / 尾 |
| `h` / `l` | `[2]` 同一列的 item 之間左右移動（表格 row、導覽列、一段文字裡的多個連結都一樣；列尾不繞）；pagetab 上走各節、**會繞**（它是選單，2026-09-22）；DevTools popup 切分頁；其他地方無 |
| 方向鍵 | 同義 |
| `1`–`2` | 直達面板 |
