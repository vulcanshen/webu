...
Hacker News 畫成 item 與文字流，`j`/`k`/`l` 走；`Enter` 在一則新聞上先問要不要開、`Space` 列出整份選單；`L` 是 Chrome 的 Cmd+L，帶著目前的 URL；`B` 是書籤 screen —— 目錄樹，`Enter` 開新分頁，載入時 URL 旁的地球在轉；那一頁有外框，所以 pagetab 填滿了 —— `Esc` 上去、`h`/`l` 走、`Enter` 列出一節裡有什麼 —— 底下的頁面是依層級鋪底色的 heading；`I` 是 DevTools，走過 Network、Storage、Console；`?` 是 help。
...
**`[W]eb`** —— `[1] Tabs` 與 `[2] Page` 並列。分頁清單一列一個 Chromium target：游標說你在哪、綠字說頁面面板正在顯示哪一個，同一個 URL 開兩個分頁會依開啟順序編號。頁面面板永遠是頁面：第一列是 URL，之後是頁面本身，畫成 **item** —— 連結、按鈕、輸入框、核取方塊、下拉選單、媒體、heading、landmark —— 段落文字在 item 之間以固定欄寬流動。`j`/`k` 一列一列走、`h`/`l` 在同一列裡左右移，所以一排連結是橫著走過去，不會被跳過。landmark 以一列帶名字的細線開頭（`▾ banner ────`），`Enter` 收合；heading 也一樣，收到下一個同級 heading 為止。頁面的外框 —— skip link、header、navigation、搜尋、側欄、footer、dialog、main 之外的東西 —— 不佔頁面，放在 URL 底下那條 pagetab 上、併成一條鏈（開頭一個漢堡 icon），一種一節、順序固定：` nav +21 ` 是這頁所有的 navigation 與裡面共幾個連結，你在裡面的哪一格寫在面板下框的 hint，塞不下的收在 `+N` 後面。`Esc` 上去、再 `Esc` 回來，`h`/`l` 走、`j` 回頁面；`Enter` 列出裡面的東西（每個 navigation 一段）、再按 `Enter` 就開；搜尋框直接開框打字，打完就問要不要送出。dialog（cookie 同意那種）自己一節、用它的名字；skip link `Enter` 就跳到它指的地方；頁內錨點連結一律這樣跳，不換頁。頁面從它的內文開始（main，沒有 main 就是第一個 heading），一整排裸連結像文字一樣流動、不再一個佔一列。頁面還在抓的時候，URL 前面的地球會轉，慢站看起來是在做事而不是卡住。heading 依層級鋪底色，h1 最明顯、h6 最淡。頁面自己的標記用終端機的屬性畫：`<strong>` 粗體、`<em>` 斜體、`<del>` 刪除線、`<mark>` 反白；`Space` 有一項把整頁轉成 markdown 放進剪貼簿。JSON、YAML、TOML、Markdown、純文字的回應畫成一個帶語法色的 code block，不用 Chrome 自己的 viewer。
...
item 上 `Enter` 就是滑鼠左鍵：輸入框直接開框打字（密碼框遮罩）、下拉選單直接拉開清單、按鈕與勾選框直接按、landmark 與 heading 直接 Collapse / Expand。連結會先問 —— confirm 裡列出連結文字與 URL —— 再按 `Enter` 才開。`Space` 是右鍵選單：連結的 Open / Open in new tab / Yank link url、輸入框的 Submit / Edit / Clear / Yank、下拉選單的 Choose，每一種都另有 Yank text 與 Inspect。panel operation：`R` 重載 · `T` 新分頁 · `P` / `N` 上一頁 / 下一頁 · `/` 搜尋 · `v` visual mode · `L` location · `A` 加書籤 · `O` outline · `I` inspect（DevTools）· `Z` zoom · `Y` yank page url · `C` 關掉這個分頁。
...
- **item 與文字流** —— 連結、按鈕、欄位、heading、landmark 是游標的落點；段落在它們之間以固定欄寬流動，表格留著欄、每格都是落點、`Enter` 看全文，code 留著行與語法色、外框是 URL 底下 pagetab 上的一節、一種一節、`Enter` 才列出裡面的東西。`h`/`l` 走一排連結，`j`/`k` 走列。
