...
- **Items and flow** — links, buttons, fields, headings and landmarks are stops for the cursor; prose flows between them at a measured width, tables keep their columns, code keeps its lines and its syntax colour, the page's chrome is one row each that lists what it holds on `Enter`. `h`/`l` walk a row of links; `j`/`k` step rows.
